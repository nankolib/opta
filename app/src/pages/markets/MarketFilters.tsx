...
export type StatusFilter = "expiring-7d" | "settled";
...
/**
 * Three-group filter row + search field for the markets section.
 *
 * Side is single-select (radio-style); asset and status are
 * multi-select (independently toggleable). Empty multi-select sets
 * mean "no filter applied" (show everything in that dimension), so
 * the filters compose by intersection across groups and union within.
 *
 * Search ⌘K affordance is decorative — focusing the input via click
 * is the only interaction, no global keyboard shortcut wired.
 */
export const MarketFilters: FC<MarketFiltersProps> = ({ state, onChange }) => {
