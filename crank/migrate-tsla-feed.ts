// crank/migrate-tsla-feed.ts
// =============================================================================
// One-shot admin script: rotate TSLA market's pyth_feed_id from Beta to mainnet.
//
// Sibling of migrate-sol-feed.ts. Same safety pattern, same idempotency guard.
// Beta hex was confirmed by an initial pre-flight read against devnet (the
// HANDOFF.md `7dac7caf…cc4e` form was an internal-truncation render, not the
// real start+end).
//
// Run (from crank/):
//   OPTA_RPC_URL="<helius>" npx ts-node -r tsconfig-paths/register \
//     migrate-tsla-feed.ts
//
// Required: keypair at ~/.config/solana/id.json must equal protocol_state.admin.
// Aborts before submitting if any pre-flight check fails.
// =============================================================================
...
async function main(): Promise<void> {
  ...
  if (currentHex === NEW_FEED_HEX) {
    console.log("");
    console.log("STOP: TSLA market is already on the mainnet feed_id.");
  ...
  console.log("  match: TSLA market now points at mainnet TSLA/USD feed");
}
